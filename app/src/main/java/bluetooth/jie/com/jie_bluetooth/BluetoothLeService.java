...
public class BluetoothLeService extends Service {
    ...
    private final static String TAG = "蓝牙模块";
    ...
    public ArrayList<BluetoothGatt> gattArrayList = new ArrayList<>();
    ArrayList<BluetoothGattCharacteristic> writeGatt = new ArrayList<>();
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                if (!address.contains(gatt.getDevice().getAddress())) {
                    address.add(gatt.getDevice().getAddress());
    ...
    public ArrayList<BluetoothGattCharacteristic> getWriteGatt() {
        return writeGatt;
    }

    public ArrayList<BluetoothGatt> getGattArrayList() {
        return gattArrayList;
    }
...
//        }
        ...
        Log.i(TAG, "broadcastUpdate: " + gatt.getDevice().getAddress());
...
//        }
        ...
        boolean isHasGat;
        if (gattArrayList.size() > 0) {
            for (BluetoothGatt gatt : gattArrayList) {
                if (gatt.getDevice().getAddress().equals(bluetoothGatt.getDevice().getAddress())) {
                    isHasGat = true;
                    Log.i(TAG, "connect: 已经存在该设备 ");
                    gattArrayList.remove(gatt);
                    break;
                }
            }
        }

        gattArrayList.add(bluetoothGatt);
    ...
    public void disconnect() {
        if (mBluetoothAdapter == null || gattArrayList.size() > 0) {
            Log.w(TAG, "BluetoothAdapter not initialized");
            return;
        }
        for (BluetoothGatt bluetoothGatt : gattArrayList) {
...
//    }
    public Boolean writeCharacteristic(BluetoothGattCharacteristic characteristic, BluetoothGatt mBluetoothGatt) {
        ...
        Log.i(TAG, "writeCharacteristic: 写入状态" + b);
        return b;
    }
...
//            Log.i(TAG, "setCharacteristicNotification: 写入状态" + b);
//        }
    }
